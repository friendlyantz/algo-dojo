...
RSpec.describe 'Solutions' do
  ...
  describe 'Part 2' do
    ...
    describe 'MaxCubesCounter' do
      it 'returns the max number of cubes' do
        input = { 1 => [{ red: 4, green: 0, blue: 3 },
                        { red: 1, green: 2, blue: 6 },
                        { red: 0, green: 2, blue: 0 }] }

        expect(MaxCubesCounter.new(input).call).to eq({ 1 => { red: 4, green: 2, blue: 6 } })
      end
    end

    context 'with example input data' do
      it 'returns correct result' do
        expect(Solution.new(example_input).solution_pt2).to eq 2286
      end
    end

    context 'with custom input data' do
      it 'returns correct result' do
        expect(Solution.new(custom_input).solution_pt2).to eq 72_970
      end
    end
  end
end
