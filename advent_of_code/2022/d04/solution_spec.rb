...
RSpec.describe 'Solutions' do
  ...
  describe 'Part 2' do
    ...
    #
    it 'tests for partial overlap' do
      expect(partial_overlap?([2..4, 6..8])).to eq false
      expect(partial_overlap?([2..3, 4..5])).to eq false
      expect(partial_overlap?([5..7, 7..9])).to eq true
      expect(partial_overlap?([2..8, 3..7])).to eq true
      expect(partial_overlap?([6..6, 4..6])).to eq true
      expect(partial_overlap?([2..6, 4..8])).to eq true
    end

    context 'example input data' do
      it 'returns correct result' do
        expect(solution_pt2(example_input)).to eq 4
      end
    end
    ...
    context 'custom input data' do
      it 'returns correct result' do
        ...
        expect(solution_pt2(custom_input)).to eq 933
      end
    end
  end
end
